...
void cleanup()
{
    if(inPPM.data) free(inPPM.data);
    if(outPPM.data) free(outPPM.data);
}
int main()
{
    read();
    if(checkerror()) {
        printf("Error during processing(read): %d\n", ERROR_STATE);
        cleanup();
        return ERROR_STATE;
    }
    handle(198,213,423,696);
    if(checkerror()) {
        printf("Error during processing(handle): %d\n", ERROR_STATE);
        cleanup();
        return ERROR_STATE;
    }
    write();
    if(checkerror()) {
        printf("Error writing output(write): %d\n", ERROR_STATE);
    }
