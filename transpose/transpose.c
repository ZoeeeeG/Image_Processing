...
//定义变量
const char *read_path ="C:\\Users\\Zoe\\OneDrive\\CPP\\PPM\\transpose\\apple.ppm";//写文件地址
const char *write_path ="C:\\Users\\Zoe\\OneDrive\\CPP\\PPM\\transpose\\apple_transpose.ppm";//同一文件夹下，写输出文件的文件名
...
void handle()
{
    int width = inPPM.height, height = inPPM.width;

    outPPM.width = inPPM.height;
    outPPM.height = inPPM.width;
    ...
    for(int y=0; y<height; y++)
    {
        for(int x=0; x<width; x++)
        {
            outPPM.data[x + y*width].r = inPPM.data[y + x*height].r;
            outPPM.data[x + y*width].g = inPPM.data[y + x*height].g;
            outPPM.data[x + y*width].b = inPPM.data[y + x*height].b; 
        }
    }
